...
if test "$SENDER" = "mouse.clicked"; then
  if test "$MODIFIER" = "none"; then
    prompt_script="var app=Application.currentApplication();app.includeStandardAdditions=true;app.displayDialog('$PROMPT', {buttons: ['Cancel', 'Empty Trash'], defaultButton: 2, withIcon: Path('$ICON')});"
